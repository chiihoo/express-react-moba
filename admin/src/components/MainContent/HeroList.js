...
function HeroList(props) {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      setLoading(true)
      const items = await http.get('/rest/heroes')
  ...
  const removeItem = async data => {
    const { confirm } = Modal
    confirm({
      title: `确定要删除英雄 “${data.name}”？`,
      content: '此操作不可逆！',
      okText: '确定',
      cancelText: '取消',
      async onOk() {
        const res = await http.delete(`/rest/heroes/${data.id}`)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <span>
            <Link to={`/admin/heroes/edit/${record.id}`}>编辑</Link>
  ...
  return (
    <div className="ItemList">
      <PageHeader onBack={() => history.goBack()} title="英雄列表"></PageHeader>
